...

{
...

    {
...

        {
            ...
            _db.Transactions.Add(new Transaction { AccountId = acc.Id, Type = "deposit", Amount = dto.Amount, Description = dto.Description });
...

        {
            ...
            if (acc.Balance < dto.Amount) return BadRequest("insufficient funds");

            acc.Balance -= dto.Amount;

            _db.Transactions.Add(new Transaction { AccountId = acc.Id, Type = "withdraw", Amount = dto.Amount, Description = dto.Description });
...

        {
            ...
            if (dto.Amount <= 0 || dto.ToAccountId == Guid.Empty) return BadRequest("invalid");

            if (dto.ToAccountId == id) return BadRequest("cannot transfer to same account");
            ...
            if (from.Balance < dto.Amount) return BadRequest("insufficient funds");

            from.Balance -= dto.Amount;

            to.Balance += dto.Amount;

            _db.Transactions.Add(new Transaction { AccountId = from.Id, Type = "transfer-out", Amount = dto.Amount, CounterpartyAccountId = to.Id, Description = dto.Description });

            _db.Transactions.Add(new Transaction { AccountId = to.Id, Type = "transfer-in", Amount = dto.Amount, CounterpartyAccountId = from.Id, Description = dto.Description });
    ...
    public class TransferDto { public Guid ToAccountId { get; set; } public decimal Amount { get; set; } public string? Description { get; set; } }
